...
class CompileMaster extends MasterCore
{
    ...
    {
        Server::delete(FEnv::get("framework.compiled")."$env/", 'directory');
        Server::create(FEnv::get("framework.compiled")."$env/", 'directory');
    }
    ...
    {
        $directory = array();
        $iterator = new DirectoryIterator(FEnv::get("framework.compiled"));
        foreach ($iterator as $dir_info) {
            if ($dir_info->isDir() && !$dir_info->isDot()) {
                ...
                array_push($directory, array("path" => $dir_info->getRealPath(), "name" => $dir_info->getFilename(),
                    ...
                    "nperms" => $octal_perms,
                    "perms" => $perms, "status" => (($this->checkFolderIsEmptyOrNot(FEnv::get("framework.compiled").
